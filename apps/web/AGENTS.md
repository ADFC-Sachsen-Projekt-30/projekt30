...
This is a web app in a monorepo containing a Preact web application:
...
- NEVER READ ANY JSON OR CSV FILE, IT WILL CRASH YOU, to look at its shape, read only the first 1024 bytes instead eg with `head -c 1024 src/data/schools.json`
...
- `npm run prettier-fix` - Enforce a uniform code formatting style
...
- **Indentation:** 2 spaces, enforced by running `npm run prettier-fix`
...
- **Formatting:** always run `npm run prettier-fix`
